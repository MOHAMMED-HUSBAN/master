...
import { submitRegistration } from '../../slice/registrationSlice'; // Import the submitRegistration action

const ProgramDetails = () => {
  const { program } = useParams();
  const navigate = useNavigate();
  const dispatch = useDispatch();
  const registrations = useSelector((state) => state.registration.registration); // Get current registrations from state
  ...
  const handleSubmit = (e) => {
    ...
    const registrationData = { ...formData, program: currentProgram.title };
    dispatch(submitRegistration(registrationData)); // Dispatch the registration action
    alert('Registration submitted successfully!');
